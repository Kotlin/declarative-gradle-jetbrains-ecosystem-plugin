...
> [!IMPORTANT]
> Declarative Gradle currently needs Gradle version `9.6.0-rc-2`.
...
```
./gradlew wrapper --gradle-version 9.6.0-rc-2
```

For the property, in the `gradle/wrapper/gradle-wrapper.properties` file, update the `distributionUrl` property as follows:

```
distributionUrl=https\://services.gradle.org/distributions/gradle-9.6.0-rc-2-bin.zip
```
